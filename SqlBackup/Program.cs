...
namespace SqlBackup
{
	internal class Program
	{
		...
		private static readonly string FolderPath = AppSettings.Default.BackupFolder;
		...
		private static void Main(string[] args)
		{
			try
			{
				using (var connection = new SqlConnection(ConnectionString))
				{
					...
					var query =
						@"SELECT db.name FROM sys.databases AS db WHERE 1 = 1 AND db.name NOT IN ( 'master', 'model', 'msdb', 'tempdb' ) AND db.state = 0 AND db.is_in_standby = 0";
		...
		private static void Archive()
		{
			foreach (var file in Directory.GetFiles(FolderPath, "*.bak", SearchOption.TopDirectoryOnly))
			{
				...
				Logger.LogInformation(
					$"Archive of file '{Path.GetFileNameWithoutExtension(file)}' completed successfully.");
			}
		}
